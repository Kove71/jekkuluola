...
def get_jokes_by_time():
    result = db.session.execute("SELECT U.username, J.content, J.created_at, J.id, SUM(V.vote) AS votesum \
        ...
        WHERE U.id=J.user_id AND V.joke_id=J.id AND J.visible=TRUE \
...
def add_joke(content):
    ...
    sql = "INSERT INTO jokes (content, user_id, created_at, visible) \
        VALUES (:content, :user_id, NOW(), TRUE) RETURNING id"
...
def get_user_jokes(username):
    sql = "SELECT U.username, J.id, J.content, J.created_at \
        ...
        WHERE U.id=J.user_id AND U.username=:username AND J.visible=TRUE \
        ORDER BY J.created_at DESC"
...
def get_jokepage_joke(joke_id):
    sql = "SELECT U.username, J.id, J.content, J.created_at \
        ...
        WHERE J.user_id=U.id AND J.id=:joke_id AND J.visible=TRUE"
